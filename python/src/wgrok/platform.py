...
async def platform_send_message(platform: str, token: str, target: str, text: str, session=None) -> dict:
    """Send a text message via the specified platform."""
    if platform == "webex":
        return await webex.send_message(token, target, text, session)
    if platform == "slack":
...
async def platform_send_card(platform: str, token: str, target: str, text: str, card: dict, session=None) -> dict:
    """Send a message with card/rich content via the specified platform."""
    if platform == "webex":
        return await webex.send_card(token, target, text, card, session)
    if platform == "slack":

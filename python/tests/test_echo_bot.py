...
class TestWgrokEchoBot:
    async def test_relays_valid_echo(self, bot_config, fake_message):
        ...
        with patch("wgrok.echo_bot.send_message", new_callable=AsyncMock) as mock_send:
            ...
            assert args[0][1] == "user@example.com"  # reply to sender
            assert args[0][2] == "myslug:hello"  # stripped echo prefix
    ...
    async def test_handles_malformed_echo(self, bot_config, fake_message):
        bot = WgrokEchoBot(bot_config)
        msg = fake_message("./echo::payload")  # empty slug

...
import { sendSlackMessage, sendSlackCard, _setSleepFn } from '../src/slack';

interface SlackCases {
  send_message: {
    expected_url: string;
...
const CASES = loadCases<SlackCases>('slack_cases.json');
...
describe('Slack send_message', () => {
  it('sends message with correct URL and auth', async () => {
    ...
    const result = await sendSlackMessage('token123', 'channel-id', 'Hello', mockFetch);
    expect(result).toEqual({ ok: true, ts: '1234.5678' });
  });
});

describe('Slack send_card', () => {
  it('sends card with blocks field', async () => {
    const mockFetch = async (): Promise<Response> => {
      return {
        status: 200,
        ok: true,
    ...
    const card = { type: 'section', text: { type: 'mrkdwn', text: '*Hello*' } };
    const result = await sendSlackCard('token123', 'channel-id', 'Hello', card, mockFetch);
    expect(result).toEqual({ ok: true, ts: '1234.5678' });
  });
});

describe('Slack retry-after handling', () => {
  it.each(CASES.retry_after.cases)('$name', async (tc) => {
    ...
    if (tc.expected_error_contains) {
      await expect(sendSlackMessage('token', 'channel', 'test', fetchFn)).rejects.toThrow(
        new RegExp(tc.expected_error_contains),
      );
    } else {
      const result = await sendSlackMessage('token', 'channel', 'test', fetchFn);
